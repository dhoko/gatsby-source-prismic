...
export declare type UnknownRecord<K extends PropertyKey = PropertyKey> = Record<K, unknown>;
export declare type IterableElement<TargetIterable> = TargetIterable extends Iterable<infer ElementType> ? ElementType : never;
export declare type JoiValidationError = InstanceType<gatsby.PluginOptionsSchemaArgs['Joi']['ValidationError']>;
...
export declare type TypePathNode = TypePath & gatsby.Node;
...
export declare type FieldConfigCreator<TSchema extends prismicT.CustomTypeModelField = prismicT.CustomTypeModelField> = (path: string[], schema: TSchema) => RTE.ReaderTaskEither<Dependencies, never, gqlc.ObjectTypeComposerFieldConfigDefinition<unknown, unknown>>;
export declare type PrismicTypePathType = PrismicSpecialType | prismicT.CustomTypeModelFieldType | prismicT.CustomTypeModelSliceType;
export declare enum PrismicSpecialType {
    Document = "Document",
    DocumentData = "DocumentData",
    Unknown = "Unknown"
}
export interface PrismicAPIDocumentNode extends prismicT.PrismicDocument, gatsby.Node {
    prismicId: string;
}
export declare type PrismicWebhookBody = PrismicWebhookBodyApiUpdate | PrismicWebhookBodyTestTrigger;
export declare enum PrismicWebhookType {
...
export declare type PrismicCustomTypeApiResponse = PrismicCustomTypeApiCustomType[];
export interface PrismicCustomTypeApiCustomType<Model extends prismicT.CustomTypeModel = prismicT.CustomTypeModel> {
    id: string;
    label: string;
    repeatable: boolean;
    json: Model;
